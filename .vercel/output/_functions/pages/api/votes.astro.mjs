...
const POST = async ({ request }) => {
  try {
    ...
    if (!scores_json || typeof scores_json !== "object" || Array.isArray(scores_json) || Object.keys(scores_json).length === 0) {
      return new Response(
    ...
    const { data: contest, error: contestError } = await supabase.from("contests").select("id").eq("id", contest_id).single();
    ...
    const { data, error } = await supabase.from("votes").insert({ contest_id, guest_name: name, scores_json }).select("id, created_at").single();
    if (error) {
